...
def subtraction_test():
    ...
    print('Tensor Subtraction Tests Succeeded.')
...
def multiplication_test():
    ...
    t1 = Tensor(np.array([[1, 2], [1, 3]]), True)
    t2 = Tensor(np.array([[1, 2], [1, 0]]), True)
    a = (t1 * t2).sum()
    a.back_prop()

    for i in range(2):
        for j in range(2):
            assert t1.grad[i][j] == t2.value[i][j]
            assert t2.grad[i][j] == t1.value[i][j]
...
def matmul_test():
    ...
    true_grad_1 = np.array([[19, 13, 12],
                            [19, 13, 12]])
    true_grad_2 = np.array([[7, 7, 7],
                            [7, 7, 7],
                            [7, 7, 7]])

    for i in range(true_grad_1.shape[0]):
        for j in range(true_grad_1.shape[1]):
            assert t1.grad[i][j] == true_grad_1[i][j]

    for i in range(true_grad_2.shape[0]):
        for j in range(true_grad_2.shape[1]):
            assert t2.grad[i][j] == true_grad_2[i][j]
